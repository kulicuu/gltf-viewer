    ...
    pub fn from_gltf(
        ...
        if let Some(color_info) = pbr.base_color_texture() {
            material.base_color_texture = Some(
                load_texture(gl.clone(), &color_info.texture(), color_info.tex_coord(), root, imp, base_path));
        }
        if let Some(mr_info) = pbr.metallic_roughness_texture() {
            material.metallic_roughness_texture = Some(
                load_texture(gl.clone(), &mr_info.texture(), mr_info.tex_coord(), root, imp, base_path));
        }
        if let Some(normal_texture) = g_material.normal_texture() {
            material.normal_texture = Some(
                load_texture(gl.clone(), &normal_texture.texture(), normal_texture.tex_coord(), root, imp, base_path));
        ...
        if let Some(occ_texture) = g_material.occlusion_texture() {
            material.occlusion_texture = Some(
                load_texture(gl.clone(), &occ_texture.texture(), occ_texture.tex_coord(), root, imp, base_path));
        ...
        if let Some(em_info) = g_material.emissive_texture() {
            material.emissive_texture = Some(
                load_texture(gl.clone(), &em_info.texture(), em_info.tex_coord(), root, imp, base_path));
        }
